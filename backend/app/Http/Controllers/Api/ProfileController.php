...
class ProfileController extends Controller
{
    ...
    {
        $data = $r->validate(['name' => 'required|string|max:120', 'phone' => ['required', 'regex:/^(?:\+94|0)7\d{8}$/'], 'preferredLocale' => 'nullable|in:en,si,ta', 'notificationEmailEnabled' => 'nullable|boolean', 'category' => 'nullable|in:student,worker,other', 'institutionOrWorkplace' => 'nullable|string|max:180', 'preferredCity' => 'nullable|string|max:100', 'minBudget' => 'nullable|integer|min:0', 'maxBudget' => 'nullable|integer|min:0', 'preferenceText' => 'nullable|string|max:1000', 'requiredFacilities' => 'array|max:30', 'aiLearningEnabled' => 'nullable|boolean', 'resetLearnedPreferences' => 'nullable|boolean', 'businessName' => 'nullable|string|max:160', 'address' => 'nullable|string|max:300', 'verificationReference' => 'nullable|string|max:120']);
        $user = $r->user();
        $user->update(['name' => $data['name'], 'phone' => $data['phone'], 'preferred_locale' => $data['preferredLocale'] ?? $user->preferred_locale, 'notification_email_enabled' => $data['notificationEmailEnabled'] ?? $user->notification_email_enabled]);
        if ($user->role === 'tenant') {
