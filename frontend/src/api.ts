...
api.interceptors.response.use(r=>r,e=>Promise.reject({message:e.response?.data?.message||'Something went wrong.',errors:e.response?.data?.errors,status:e.response?.status}));
