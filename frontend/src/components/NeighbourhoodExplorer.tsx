...
export function NeighbourhoodExplorer({latitude,longitude,label,places,autoOpen=false}:{latitude:number;longitude:number;label:string;places:NearbyPlace[];autoOpen?:boolean}){
  ...
  return <section className="sb-detail-section sb-neighbourhood" id="neighbourhood-explorer">
    ...
    {open&&<div className="sb-nearby-explorer"><div className="sb-ai-method"><span><i className="bi bi-cpu"/></span><div><strong>How this result is produced</strong><p>Bodim AI helps match the property to your request. Nearby-place distances are then calculated from coordinates, so the map remains clear and verifiable.</p></div></div><div className="sb-place-filters" role="group" aria-label="Filter nearby places"><button className={selected==='all'?'active':''} onClick={()=>setSelected('all')}><i className="bi bi-grid"/> All essentials</button>{available.map(type=>{const item=details[type];return <button className={selected===type?'active':''} style={{'--place-colour':item.colour} as React.CSSProperties} key={type} onClick={()=>setSelected(type)}><i className={`bi ${item.icon}`}/> {item.label}</button>})}</div><div className="sb-nearby-layout"><div className="sb-nearby-map-wrap"><MapView latitude={latitude} longitude={longitude} label={label} places={places} highlightedType={selected}/><div className="sb-map-legend"><span><i/> Approximate property area</span>{available.map(type=><span key={type}><i style={{background:details[type].colour}}/> {details[type].label}</span>)}</div></div><div className="sb-nearby-list">{visible.map(place=>{const item=details[place.type]||{label:place.type.replaceAll('_',' '),icon:'bi-geo-alt',colour:'#16745e'};return <article key={place.type}><span style={{'--place-colour':item.colour} as React.CSSProperties}><i className={`bi ${item.icon}`}/></span><div><small>{item.label}</small><strong>{place.name}</strong><p>Straight-line distance from the approximate property marker</p></div><b>{distance(place.distanceM)}</b></article>})}</div></div><small className="sb-distance-disclaimer"><i className="bi bi-info-circle"/> Distances are straight-line estimates, not live walking or traffic routes. Confirm the route before making a decision.</small></div>}
  </section>;
}
