...
describe('AiChatbot',()=>{it('opens the transparent ranking interface from the floating launcher',()=>{render(<MemoryRouter><AiChatbot/></MemoryRouter>);fireEvent.click(screen.getByRole('button',{name:/open bodim ai assistant/i}));expect(screen.getByRole('dialog',{name:/bodim ai assistant/i})).toBeInTheDocument();expect(screen.getByText(/strict matching \+ transparent ranking/i)).toBeInTheDocument();expect(screen.getByPlaceholderText(/campus, budget, wifi, ac, parking/i)).toBeInTheDocument();expect(screen.getByRole('button',{name:/university of moratuwa/i})).toBeInTheDocument()})});
